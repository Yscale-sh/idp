# Reference environments & promotion: dev -> prod (stage deferred)

The environment and promotion design (decided 2026-06-12).
Status in the reference instance: **dev -> prod is the proven path; stage is scaffolded but
deferred.** A fork can keep these env names or replace them with its own. See "What's left to build"
for the gaps.
...
```
DEV CLUSTER (k3s/on-prem or LAN)                PROD CLUSTER (k3s on cloud nodes,
...
└─ shared infra modules (dev umbrella owns)     ├─ optional burst nodes: yscale
                                                └─ DR: etcd→R2 6h (baked in image)
```

- **One cluster, one FluxInstance.** Stage does NOT get its own Flux. The
  dev cluster's FluxInstance gains a second Flux Kustomization applying
...
Why not two Flux instances on the same dev cluster (dev + stage):
...
Why not ONE global Flux (a dev-cluster hub pushing to prod via kubeconfig):
1. **Prod must self-heal with zero dependency on the dev cluster.** Pull-based
   Flux on the prod cluster reconciles from GitHub alone; the only coupling to
   dev is a git branch.
...
|---|---|---|---|
| cluster | dev k3s cluster | dev k3s cluster | cloud k3s cluster (Linode in the reference instance) |
...
is added to the dev cluster: flip the seam and provide the tunnel when that is
wanted.
...
1. **dev (continuous, automatic):** push to your app repo. The in-cluster
   ...
   render core, then commits and pushes to `main`. The dev Flux reconciles.
...
- **Cluster existence is not Flux's at all.** Nodes, the k3s image, cloud
  instances, and etcd are provisioned and rebuilt out-of-band from the node image
  plus etcd snapshots. Flux only manages what runs *on* the cluster. A full
  cluster rebuild (Gate 0 restore drill) never touches deployment state, and vice
  versa.
...
**Current state:** Umbrella naming uses `platform-<env>` (dev keeps the legacy bare name) via `render.platformReleaseName`. Stage is scaffolded on the dev cluster: a `platform-stage` umbrella (0 modules) plus `clusters/dev/sync-stage.yaml` (a second Flux Kustomization applying `clusters/stage`, same FluxInstance — one object, second env). `idpctl promote <workload> <env> --from <env> -f deploy.yaml` is built and proven (`cmd/idpctl/promote_cmd.go`): provenance from the source umbrella (refuses unknown workloads and mutable tags), gate via `promotion: {from: <env>}` (`--force` overrides), prints the `flux.branch` commit step (CI-agnostic); `environments/prod/cluster.yaml` declares `promotion.from: dev`, so prod accepts a dev digest. The shipper runs per environment: dev's instance (`registry.env: dev`) commits `main`; prod runs its own (`registry.env: prod`, `platformBranch: prod`), brought online app by app, committing the `prod` branch; the digest-forward `idpctl promote <app> prod --from dev` and the prod shipper coexist during bring-up. Prod bring-up is rehearsed: k3s cluster → flux-operator + `clusters/prod/flux-instance.yaml` (branch prod) → `platform-ssm` store → promote apps one by one, flip the CF Tunnel route at cutover (rollback = flip back). The one remaining TODO in `promote` is `deploy.yaml`-at-built-SHA auto-fetch.

The stage promotion path is the one thing not yet active: flip `promotion.from: stage` in `environments/prod/cluster.yaml` and promote through stage when that hop is wanted.
...
- **Cluster state:** k3s etcd snapshots every 6h, local plus R2 (`ETCD_S3_*`
  baked into the node env). Restore: `k3s server --cluster-reset
  --cluster-reset-restore-path=...` (use your cluster image/runbook).
