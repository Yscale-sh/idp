...
> **Status: single-store model implemented; per-tenant model still the target.** Captured
> 2026-06-06; updated 2026-06-14. The reference skeletons use `platform-local`
> (Kubernetes-provider) for dev and `platform-ssm` for prod. The author's private
> instance has also proven an SSM-backed platform-infra store, but a fork should name
> and scope its own stores. The PER-TENANT model below (platform mints per-app keys,
> per-app SecretStore + IAM derivation) remains the target for multi-tenant use. This
> doc records the decision so it doesn't get lost. Naming conventions live in
...
A developer's `deploy.yaml` is a shopping list: it declares *needs*, `db: postgres`,
`cache: redis`, `storage: {type: s3}`, secrets at `/apps/<app>/<env>`. The platform's
job is not just to *read* those secrets, it is to **provision the backing resource and
mint its credential**:
...
the platform derives the implementation. Here the *credential* is part of that
implementation.

**How this maps to the implemented single-store path.** The *write* side is largely already real:
**CI/CD populates SSM** (`/apps/<app>/<env>/*`) on deploy. The missing half in any
dev cluster is the *read* side: **ESO pulls those SSM params into local k3s Secrets**.
...
The single-store shape is **one `platform-ssm` ClusterSecretStore** backed by **one AWS
...
| **Per-app reader** | the app at runtime (its SecretStore) | **read-only**, scoped to **only** `/apps/<that-app>/<env>/*` | least-privilege; App A physically cannot read App B's secrets |
...
The key part: `idpctl render` should also **emit the per-app least-privilege
...
1. **Secret population, mostly solved.** Getting credential *values* into SSM is
   ...
   only gap is the *read* half on the dev cluster: install **ESO** + the SecretStore so
   those SSM params materialize into local k3s Secrets. This is the near-term,
   buildable work, nothing to "provision," only wiring the read path.
2. **Resource provisioning, later.** Having the platform *create the backing
   ...
   resource's creds to SSM is a further step. Today `dev-postgres` is an
...
- **Reproducible.** The cred lives in SSM, not trapped in a live cluster; it survives a
  full rebuild (goal #1).
...
- **Dev is practically identical to prod for creds.** The secret wiring, rotation, and
  ESO materialization you exercise in dev are exactly what runs in prod.
...
- **Platform-infra secrets** → e.g. `/platform/*` (or `/platform-infra/*`), **one scoped
...
- ✅ In the author's private instance, a scoped platform-infra reader exists
  (`ssm:Get*` on the platform-infra prefix, read-only). A fork should create its own
  equivalent if it wants SSM-backed infra secrets in dev.
- ✅ External-Secrets Operator is represented by the `external-secrets` module.
  Dev runtime secrets use the `platform-local` Kubernetes-provider store; prod specs
  the `platform-ssm` SSM store. Platform-infra secrets such as GHCR pull credentials,
  image-builder credentials, and Loki/R2 credentials should use the fork's scoped
