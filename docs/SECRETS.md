...
> the `homelab-ssm` ClusterSecretStore (AWS SSM, `eso-homelab` reader) is live, so app
...
> target for when prod / multi-tenant is actually on the table. This doc records the
