...
> default ingress that replaces ingress-nginx at $0 vs $20/mo, and self-hosted Postgres) is now
...
2. **cert-manager ClusterIssuer** (Let's Encrypt prod + staging): already installed, so you only
   add the issuers and annotate Ingresses. Auto-TLS for every host, no manual certs.
