...
> Historical note: this is an early DX exploration. The push-based, Helm-upgrade, Docker-Hub
> `stackmaster/*` model described below was the starting proposal. It was later superseded and
> folded into the contract-based platform (a per-app `deploy.yaml` shopping list, `idpctl render`,
> and Flux as the only writer). The text is preserved as written to record the reasoning at the
> time; do not treat its technical claims as the current design.

Answers three questions: (1) what do I do to deploy, (2) is there a platform repo that's the
...
becomes a step in `scripts/new-app`, with no self-hosted identity provider needed.
→ *Heavier alternative* if you outgrow that: a self-hosted OIDC issuer (**Zitadel / Authentik /
Keycloak**) doing OAuth2 client-credentials with per-service RBAC. More capable, materially more ops
