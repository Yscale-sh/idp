...
|---|---|---|
| **Cloudflare Tunnel** | `TUNNEL_TOKEN` | platform-owned (shared `cloudflared` or per-app token); carshowdb already uses it |
| **Tailscale egress** | `TAILSCALE_AUTH_KEY/HOSTNAME`, `TS_AUTHKEY/TS_HOSTNAME/TS_KUBE_SECRET/TS_USERSPACE/TS_ACCEPT_DNS` | inject **only when `tailscaleEgress: true`** (e.g. ecommerce→GoUS DB). Most apps **drop these** once in-cluster — they were only there to reach the cluster/DB over Tailscale |
...
## Tier D — Data stores (point at in-cluster Postgres/Redis; Mongo is the exception)
| Engine | Vars | Apps |
|---|---|---|
...
| **MongoDB** | `MONGO_USERNAME`, `MONGO_PASSWORD` | **unbelievable** ← not Postgres; self-hosted-PG plan doesn't cover it |
...
- `GOOGLE_CLIENT_*` (end-user login) → **stays per-app** (external-provider OAuth; platform stores/syncs, can't mint).
- `SESSION_SECRET`, `SECURE_COOKIES`, `COOKIE_DOMAIN`, `TRUSTED_AUTH_PROXIES`, `ALLOW_UNSIGNED_WEBHOOKS`,
  `ENABLE_DEBUG_ENDPOINTS` → app session/security config; keep, but `TRUSTED_AUTH_PROXIES` should
...
- **unbelievable runs MongoDB** — the self-hosted-Postgres plan doesn't cover it; decide: self-host Mongo, or migrate it to Postgres, or keep external.
- **ecommerce DB = GoUS over Tailscale** — its `tailscaleEgress: true` stays until/unless that DB moves in-cluster.
