...
type Registry struct {
	...
	// CloudflareEnabled gates the in-process tunnel auto-provisioner. When true
	// AND CLOUDFLARE_API_TOKEN + CLOUDFLARE_ACCOUNT_ID are set in the environment,
	// the shipper calls EnsureTunnel/SetIngress/SyncHosts after each successful
	// ship for apps that have CF-zone public routes. Failures are soft (logged,
	// never abort the ship). Default false — rendered from values.cloudflare.enabled.
...
func main() {
	...
	for {
		for _, app := range reg.Apps {
			...
			maybeProvisionTunnel(ctx, reg, app, sha, token, k)
		}
		time.Sleep(interval)
	}
}
