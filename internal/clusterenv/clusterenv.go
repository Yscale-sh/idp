...
type Config struct {
	...
	// CloudflareZone selects which public route hosts should use the Cloudflare
	// Tunnel when an env also exposes LAN routes. Empty means every public route
	// is tunnel-eligible, preserving the prod behavior where public exposure is
	// entirely Cloudflare-backed.
...
// IsCloudflareHost reports whether a (composed) route host falls under the env's
// CloudflareZone — i.e. should be served via the Cloudflare Tunnel rather than
// the LAN path. An empty zone means every host is Cloudflare-eligible (the prod
// behavior, where public exposure is entirely Cloudflare-backed). Matching is
// case-insensitive, ignores trailing dots, and requires a label boundary
// ("notyscale.sh" does not match zone "yscale.sh").
func (c *Config) IsCloudflareHost(host string) bool {
	if c == nil || c.CloudflareZone == "" {
		return true
	}
	h := strings.TrimSuffix(strings.ToLower(strings.TrimSpace(host)), ".")
	z := strings.TrimSuffix(strings.ToLower(strings.TrimSpace(c.CloudflareZone)), ".")
	if z == "" {
		return true
	}
