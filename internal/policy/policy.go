...
var (
	...
	ErrCFAccess       = errors.New("policy: dev Cloudflare exposure must be Access-gated")
)
...
func Check(in Input) Violations {
	...
	// (6) CF Access: a public route in a CloudflareZone env must declare access
	// gating — the *.yscale.sh wildcard Access app enforces it at the edge.
	vs = append(vs, checkCFAccess(app, in.Cluster)...)
...
// checkCFAccess enforces the Access-gating contract for Cloudflare-exposed
// routes: in an env where CloudflareZone is set, any public route whose
// composed host falls under that zone MUST declare access.humans:true OR
// access.serviceToken:true. The *.yscale.sh wildcard Cloudflare Access app
// enforces authentication at the edge; an unprotected host reaching the
// tunnel without an Access declaration is a contract violation.
//
// Envs with no CloudflareZone (prod today, where every public host is CF-backed
// and the zone is the whole domain) are unchanged — they already require the
// Access config via the Access app setup, not via this policy check.
func checkCFAccess(app appconfig.App, c *clusterenv.Config) Violations {
	var vs Violations
	if c == nil || c.CloudflareZone == "" {
		return vs
	}
	for _, r := range app.Routes {
		if !r.Public || r.Host == "" {
		...
		if !c.IsCloudflareHost(host) {
			// .local / non-CF routes are LAN-only — no Access in front.
			continue
		}
		if !r.Access.Humans && !r.Access.ServiceToken {
			vs = append(vs, &Violation{KindCFAccess,
				fmt.Sprintf(
					"dev Cloudflare exposure is Access-gated by contract: "+
						"route host %q must declare routes[].access.humans or routes[].access.serviceToken "+
						"(the *.%s Access app enforces it at the edge)",
					host, c.CloudflareZone,
				)})
		}
	}
	return vs
