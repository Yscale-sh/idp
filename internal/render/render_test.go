...
func devCloudflareSplitCluster() *clusterenv.Config {
	c := devCluster()
	c.Zones = []string{"carshowdb.local", "*.local", "yscale.sh", "*.yscale.sh"}
...
func TestGolden_DevCloudflareZoneSplit(t *testing.T) {
	goldenCase(t, "dev-zone-split.deploy.yaml", "dev", devCloudflareSplitCluster(),
		"ghcr.io/yscale-sh/myapp:dev-zone", "dev-zone-split.dev.golden.yaml")
}
