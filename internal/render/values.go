...
// cfRoutes returns public routes whose host is eligible for Cloudflare Tunnel
// exposure in this env. An empty cloudflareZone means all public routes are
// eligible, which preserves prod's historical "public == tunnel" behavior.
func cfRoutes(app appconfig.App, c *clusterenv.Config) []appconfig.Route {
	out := make([]appconfig.Route, 0, len(app.Routes))
	for _, r := range app.Routes {
		if !r.Public || r.Host == "" {
			continue
		}
		if c.IsCloudflareHost(r.Host) {
			out = append(out, r)
		}
	}
	return out
...
func lanPublicRoutes(app appconfig.App, c *clusterenv.Config) []appconfig.Route {
	out := make([]appconfig.Route, 0, len(app.Routes))
	for _, r := range app.Routes {
		if !r.Public || r.Host == "" {
			continue
		}
		if envProvidesTunnel(c) && c.IsCloudflareHost(r.Host) {
			continue
		}
		// If cloudflareZone is set, any host matching it belongs to the Cloudflare
		// Tunnel and must not fall back to LAN exposure, even if the tunnel seam
		// is disabled.
		if c != nil && c.CloudflareZone != "" && c.IsCloudflareHost(r.Host) {
			continue
		}
		out = append(out, r)
	}
...
// Cloudflare-eligible public routes, or nil when the tunnel does not apply (so
// the values field is omitted and the render stays byte-identical). In prod,
// every public route is eligible. In dev, cloudflareZone lets the tunnel coexist
// with LAN exposure by selecting only the public hosts under that zone.
func buildTunnel(app appconfig.App, c *clusterenv.Config) *TunnelValues {
	if !envProvidesTunnel(c) {
		return nil // common dev-local path: short-circuit before any route scan
	}
	routes := cfRoutes(app, c)
	if len(routes) == 0 {
		return nil
	}
	svc := fmt.Sprintf("http://localhost:%d", app.Runtime.Port)
	ingress := make([]TunnelIngressValues, 0, len(routes))
...
//     fields: a public route means "expose to users". In a mixed dev env, only
//     the public hosts not selected by cloudflareZone derive LAN exposure. The
//     hostname comes from the (env-composed) route host; the IP is auto-assigned
//     from the env's LanPool.
//
...
func buildLanExpose(app appconfig.App, c *clusterenv.Config) *LanExposeValues {
	...
	lanRoutes := lanPublicRoutes(app, c)
	derived := !explicit && len(lanRoutes) > 0 && envProvidesLAN(c)
	if !explicit && !derived {
		return nil
	}
	...
	if derived {
		if lan.Host == "" {
			lan.Host = publicHostList(lanRoutes) // every LAN public host -> external-dns name(s)
		}
		if lan.IP == "" && lan.Pool == "" {
...
// envProvidesTunnel reports whether the env can serve eligible public routes over
// a real Cloudflare Tunnel (the cloudflared sidecar + a pinned TUNNEL_TOKEN).
// Defaults to the ssm/prod backend; an env can set seams.tunnel:false (e.g. a
// LOCAL prod stand-in with no real Cloudflare) so public routes render with NO
// sidecar and NO token — ClusterIP only, and a deploy can never open a tunnel.
func envProvidesTunnel(c *clusterenv.Config) bool {
	return c != nil && c.EffectiveSeams().Tunnel
}

// publicHostList is the comma-joined list of public route hosts (already
// env-composed by render time) that should point at the LAN LoadBalancer.
// external-dns accepts a comma-separated hostname annotation, so every LAN public
// host gets a DNS name pointing at the one MetalLB VIP — not just the first.
func publicHostList(routes []appconfig.Route) string {
	var hosts []string
	for _, r := range routes {
		if r.Host == "" {
			continue // an unfiltered slice must not produce a dangling comma
		}
