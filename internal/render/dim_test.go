...
func TestDim_MultiComponentIngest(t *testing.T) {
	...
	if uiRes.Values.LanExpose == nil || !uiRes.Values.LanExpose.Enabled || uiRes.Values.LanExpose.IP != "192.0.2.20" {
		t.Errorf("ui should LAN-expose on 192.0.2.20, got %+v", uiRes.Values.LanExpose)
	}
