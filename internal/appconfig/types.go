...
// applies to any route served over a Cloudflare Tunnel. In prod every public
// route goes through the tunnel. In dev, public routes under the env's
// CloudflareZone (e.g. *-dev.yscale.sh) also go through the tunnel and REQUIRE
// an access declaration (enforced by policy); public routes on .local remain
// network-gated at the LAN layer with no Cloudflare in front.
type Access struct {
...
// <app>-<env>-<tool> (e.g. sample-dev-postgres, media-dev-redis). A second
