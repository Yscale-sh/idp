...
cheap, correct path the *default*.

**Status:** reference edition of the Yscale IDP. The included skeletons model a **dev to prod**
promotion path (digest-forward — prod runs the same artifact digest that ran in dev, never a rebuild — via `idpctl promote --from dev`), with a **stage** tier scaffolded
but deferred for now. In the reference topology, dev and prod are k3s clusters and prod runs on
Linode; a fork points the same contracts at its own clusters. The CLI is **`idpctl`** (Go module
`github.com/yscale-sh/idp`). Apache-2.0.
...
render` writes the rendered umbrella (the umbrella: ONE HelmRelease at `clusters/<env>/platform.yaml`
that installs `charts/cluster`, which templates one isolated HelmRelease per app) into `clusters/<env>/`
in your fork. You do not install idp,
...
```
 ...
 (the developer's   (CLI: validate /     (ONE umbrella HelmRelease whose          (the only
...
For the full visual guide to this pipeline, see [`docs/DIAGRAMS.md`](docs/DIAGRAMS.md).
...
- **Operator, once:** install the **Flux Operator** and apply one `FluxInstance`
  ...
  the **umbrella HelmRelease** (`platform.yaml`) — the Helm-native replacement for the old Argo CD
  app-of-apps. `disableWait` isolates a failing app so it cannot wedge its siblings.
...
The reference prod target is a self-provisioned k3s cluster on Linode (hardened nodes, Cilium,
embedded etcd, kube-vip, and optional etcd snapshots to R2). prod exposure is
...
> **Editor tip:** add `# yaml-language-server: $schema=https://raw.githubusercontent.com/yscale-sh/idp/main/schemas/deploy.schema.json`
> as the first line of a `deploy.yaml` to get inline validation in editors with the YAML language server.

| Field | What it does |
|---|---|
| `component` | One **product**, many components (e.g. yscale-media's `api` / `scanner` / `ui`). Each renders an isolated HelmRelease with component-aware names (`<app>-<component>`); SSM/secret roots stay app-level so siblings share. |
...
See `examples/carshowdb` (a single-service app), `examples/hello-dev` (a dev app exposed
behind Cloudflare Access login), and `examples/yscale-media`, a **multi-component media
server** (api + scanner + ui) that adds an *optionally-distributed sharded transcoder*: a
`replicas: 0` GPU worker pool that scales out to encode one stream across many nodes. It
exercises nearly the whole contract (`build` submodules, shared
Postgres + Redis, an iGPU, an NFS media library, a LAN-exposed UI, GPU `extraLimits`, four
volume kinds, `connectsTo`, `expose.lan.host`).
...
modules:
  ...
  unifi-controller: { enabled: false } # site-local service: modules aren't just "platform" tech
```
...
| `examples/{carshowdb,hello-dev,yscale-media}` | onboarded apps: a single service, a dev app behind Cloudflare Access, and a multi-component product |
...
git diff -- clusters/dev/platform.yaml   # see what render just wrote
git add clusters/dev/platform.yaml && git commit -m "deploy: add carshowdb"
# in a real fork, pushing this commit IS the deploy: Flux sees it and reconciles
make e2e          # spins an ephemeral k3d cluster, applies, asserts rollout, tears down (SKIPs if k3d absent)
```

Flux installs the umbrella chart, which fans out into an isolated HelmRelease for this app — no `kubectl apply`, no `helm upgrade`.
...
2. **Generate your own env** instead of inheriting this instance's: `idpctl init --env dev`
   writes a fresh `environments/dev/cluster.yaml` wired to *your* `idp.yaml` (no site-local IPs,
...
the secrets trust model ([`SECRETS.md`](docs/SECRETS.md)), and env-var tiers
([`ENV.md`](docs/ENV.md)). Postgres backups are CNPG barman→R2 (chart
...
One ingress path, ClusterIP-only apps, self-hosted data stores, and elastic cloud burst only when load demands it.
